...
#2.Compute item mean
#3.Get input matrix for SVD after original_matrix - item mean.
...
norm_item_rating_mat = get_item_mean_mat(user_rating_mat)
U,s,V = np.linalg.svd(norm_item_rating_mat, full_matrices=False)

#To make 1-d matrix diagonal matrix
#S = np.diag(s)
...
item_mean = compute_mean(user_rating_mat[:,iid])
ans = item_mean + t_sim
