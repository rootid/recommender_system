...
def get_user_item_mean_mat (user_mat) :
   ...
   #    for j in range(col) :
   #        if (user_mat[i][j] != 0) :
   #            user_norm_rating_mat[i][j] = user_mat[i][j] - global_mean
   for j in range(col) :
       item_mean = compute_mean(user_mat[:,j])
       #item_mean = compute_mean(user_norm_rating_mat[:,j])
       for i in range(row):
           #DO NOT CONSIDER rating matrix with zero
           if (user_mat[i][j] != 0) :
               user_norm_rating_mat[i][j] = user_mat[i][j] - item_mean
               #user_norm_rating_mat[i][j] = user_norm_rating_mat[i][j] - item_mean
   for i in range(row) :
       user_mean = compute_mean(user_norm_rating_mat[i,:])
       for j in range(col) :
           if (user_mat[i][j] != 0) :
               user_norm_rating_mat[i][j] = user_norm_rating_mat[i][j] - user_mean
   return user_norm_rating_mat 

def get_u2 (user_mat,uid) :
   (row,col) =  user_mat.shape
   user_norm_rating_mat = np.zeros((row,col))
   for j in range(col) :
       item_mean = compute_mean(user_mat[:,j])
       for i in range(row):
           ...
           if (user_mat[i][j] != 0) :
               user_norm_rating_mat[i][j] = user_mat[i][j] - item_mean
   return compute_mean(user_norm_rating_mat[uid,:])


#Algorithm
#1.create all necessary lookups
#2.Compute item_mean ,user_mean
#3.Get input matrix for SVD after original_matrix - item_mean - user_mean.
...
norm_user_item_rating_mat = get_user_item_mean_mat(user_rating_mat)
U,s,V = np.linalg.svd(norm_user_item_rating_mat, full_matrices=False)
S = np.diag(s)
...
sim_ = m1*(m2)*(m3)
t_sim = sum(sim_)
mu1 = compute_mean(user_rating_mat[:,iid])
mu2 = get_u2 (user_rating_mat,uid)
u_item_mean = (mu1 + mu2) 
ans = u_item_mean + t_sim
